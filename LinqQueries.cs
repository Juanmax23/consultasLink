public class LinqQueries
{
    ...
    public IEnumerable<Book> LivrosQueEmiecenConUnNombre()
    {
        //Liros que el titulo empiece con Java
        return librosCollection.Where(p => p.Title.StartsWith("Java")).ToList(); 
    }
    ...
    //Supongamos que el libro tubiera un objeto en su clase o una lista de objetos 
    //Si me quiero traer estos a travez de consultas link seria 
    //public Book TodosLosLibrosConSusAutores(string title)
    //{
    //    return librosCollection.Where(l => l.Title == title ).Include(h => h.Autor);
    //}
    public Book BuscarLibroYSiDosTienenElMismoTituloTeDaUnaException(string title)
    {
        return librosCollection.SingleOrDefault(a => a.Title == title);
